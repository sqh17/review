...
- [react部分](#react部分)
  ...
  - [什么是prop drilling？](#什么是prop-drilling)
...
组件的 state 属性是只读的，只能通过 setState() 方法来修改。setState() 是大部分下是一个异步的方法，用于告诉 React 需要更新组件的状态，并触发一次重新渲染。setState() 方法的调用有两个作用：一是更新组件状态，二是告诉 React 需要重新渲染组件
setState() 方法的机制如下：
...
在react17之后，setState将异步更新改为同步更新，调用setState后，组件状态会立刻更新，并且在调用setState的代码块中可以立即访问最新状态，这种改变为了让setState更加直观，并减少由异步更新而导致的潜在问题，特点如下：

1. setState是同步更新的，调用setState后，组件状态会立刻更新，并且在调用setState的代码块中可以立即访问最新状态
...
总结起来，React 17 中的 setState 是同步更新的，并通过批量更新和基于队列的机制来提高性能和保证状态的一致性。在开发中，可以放心地使用 setState 来更新组件的状态，而且可以立即访问到最新的状态

### 为什么不直接更新state

因为这不符合react的设计理念，如果直接修改state，可能会导致状态的不一致，所以会采用setState的方式来修改state，setState方法不仅能更新state，还能通知react，组件的状态已经发生了变化，需要重新渲染，这样就能保证统一性和高效性
...
react的事件处理方式就是在组件上添加事件监听器，当事件触发时，会生成一个合成事件对象，这个合成事件对象就是基于原生浏览器的事件对象+react的一些方法和属性，这个合成事件对象也可以直接获取到原生的属性和方法，在调用事件监听器时，这个合成事件对象会传递给事件监听器，但注意的是，react对事件进行了事件委托，所以说不是直接绑定在真实dom上的，而是注册在 React 的事件系统中，当事件触发时，React 会冒泡向上寻找组件树中注册了该事件处理器的组件，并将事件委派给该组件处理。经过这样的委托处理，React 保证了整个事件系统的高效运作，接下来就是更新组件，在事件处理器执行过程中，可能会修改组件的状态或者触发其他某些操作，从而需要重新渲染组件。最根本的机制就是事件委托来实现的，具体来说，当你在某个组件上注册了事件处理函数时，React会把该事件委托给祖先元素来处理。在事件冒泡阶段，React会通过委托机制把事件传递给最近的祖先组件，从而实现事件处理和状态更新的目的
...
在react应用中，将props通过多层次传递最底层，当组件层级较多时，如果某个深层组件需要使用顶层组件传递下来的 props，但中间层的组件并不需要这些 props，为了让最深层的组件获取到需要的 props，这些 props 就需要一层层地通过中间组件进行传递，即 prop drilling
缺点：
